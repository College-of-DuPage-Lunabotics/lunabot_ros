...
private:
  ...
  void request_localization()
  {
    if (!localization_client_->wait_for_action_server(std::chrono::seconds(1))) {
      RCLCPP_WARN_ONCE(
        this->get_logger(),
        "\033[1;LOCALIZATION ACTION SERVER NOT AVAILABLE.\033[0m");
      return;
    }
  ...
  void handle_localization_result(const GoalHandleLocalization::WrappedResult & result)
  {
    if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
      ...
      RCLCPP_INFO_ONCE(
        this->get_logger(), "\033[1;32mLOCALIZATION COMPLETE. INITIAL POSE: [%.2f, %.2f]\033[0m",
  ...
  void request_navigation()
  {
    if (!navigation_client_->wait_for_action_server(std::chrono::seconds(1))) {
      RCLCPP_WARN_ONCE(
        this->get_logger(), "\033[1;33mNAVIGATION ACTION SERVER NOT AVAILABLE.\033[0m");
      return;
    }
    ...
    RCLCPP_INFO_ONCE(
      this->get_logger(), "\033[1;36mSENDING NAVIGATION GOAL TO EXCAVATION ZONE...\033[0m");
  ...
  void handle_navigation_result(const GoalHandleNavigate::WrappedResult & result)
  {
    if (result.code == rclcpp_action::ResultCode::SUCCEEDED) {
      RCLCPP_INFO_ONCE(
        this->get_logger(), "\033[1;32mEXCAVATION ZONE REACHED. REQUESTING EXCAVATION...\033[0m");
      start_excavation_ = true;
    } else if (result.code == rclcpp_action::ResultCode::ABORTED) {
      RCLCPP_ERROR(
        this->get_logger(), "\033[1;31mGOAL ABORTED, UNABLE TO REACH EXCAVATION ZONE\033[0m");
    }
  }
  ...
  void request_excavation()
  {
    if (!excavation_client_->wait_for_action_server(std::chrono::seconds(1))) {
      RCLCPP_WARN_ONCE(
        this->get_logger(), "\033[1;33mEXCAVATION ACTION SERVER NOT AVAILABLE.\033[0m");
      return;
    }
