...
/**
 * @class IMURotator
 * @brief Converts IMU data from NED to ENU frame for both D455 and D456
 * cameras.
 */
class IMURotator : public rclcpp::Node
{
  public:
    ...
    IMURotator() : Node("imu_rotator")
    {
        ...
        d456_imu_subscriber_ = this->create_subscription<sensor_msgs::msg::Imu>(
            "d456/imu", qos, std::bind(&IMURotator::d456_imu_callback, this, std::placeholders::_1));
  ...
  private:
    /**
     * @brief Callback function for transforming and publishing D455 IMU data from
     * NED to ENU frame.
    ...
    /**
     * @brief Callback function for transforming and publishing D456 IMU data from
     * NED to ENU frame.
     * @param msg The received IMU message.
     */
     ...
     */
    void transform_imu_data(const sensor_msgs::msg::Imu::SharedPtr msg,
