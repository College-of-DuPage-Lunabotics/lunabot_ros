...
class HardwareMonitor : public rclcpp::Node
{
  public:
     ...
     */
    HardwareMonitor()
        : Node("hardware_monitor"), d456_valid_(false), d455_valid_(false), lidar1_valid_(false), lidar2_valid_(false)
    {
        ...
        lidar1_timer_ =
            create_wall_timer(std::chrono::seconds(5), [this]() { check_connection(lidar1_valid_, "LIDAR A3"); });
        lidar2_timer_ =
            create_wall_timer(std::chrono::seconds(5), [this]() { check_connection(lidar2_valid_, "LIDAR S2L"); });
