 ...
 * @brief Monitors the hardware connections and status of sensors (D456, D455,
 * and LIDARs).
 */
...
public:
   ...
   */
  HardwareMonitor()
  : Node("hardware_monitor"), d456_valid_(false), d455_valid_(false), lidar1_valid_(false),
    lidar2_valid_(false)
  {
    ...
    lidar1_subscriber_ = create_subscription<sensor_msgs::msg::LaserScan>(
      ...
      [this](const sensor_msgs::msg::LaserScan::SharedPtr) {
        reset_timer(lidar1_timer_, lidar1_valid_);
      });

    lidar2_subscriber_ = create_subscription<sensor_msgs::msg::LaserScan>(
      "scan2", rclcpp::QoS(10).reliable(),
      [this](const sensor_msgs::msg::LaserScan::SharedPtr) {
        reset_timer(lidar2_timer_, lidar2_valid_);
      });

    d456_timer_ = create_wall_timer(
      std::chrono::seconds(5), [this]() {
        check_connection(d456_valid_, "D456");
      });
    d455_timer_ = create_wall_timer(
      std::chrono::seconds(5), [this]() {
        check_connection(d455_valid_, "D455");
      });
    lidar1_timer_ =
      create_wall_timer(
      std::chrono::seconds(5), [this]() {
        check_connection(lidar1_valid_, "LIDAR A3");
      });
    lidar2_timer_ =
      create_wall_timer(
      std::chrono::seconds(5), [this]() {
        check_connection(lidar2_valid_, "LIDAR S2L");
      });

    all_sensors_timer_ = create_wall_timer(
  ...
  void check_connection(bool & valid_flag, const std::string & sensor_name)
  {
    if (!valid_flag) {
      RCLCPP_WARN(
        get_logger(), "\033[0;36m%s:\033[0m \033[1;31mNO CONNECTION, CHECK CABLE\033[0m",
        sensor_name.c_str());
    }
    valid_flag = false;
  }
...
private:
  ...
  rclcpp::Subscription<sensor_msgs::msg::LaserScan>::SharedPtr lidar1_subscriber_,
    lidar2_subscriber_;
  rclcpp::TimerBase::SharedPtr d456_timer_, d455_timer_, lidar1_timer_, lidar2_timer_,
    all_sensors_timer_;
