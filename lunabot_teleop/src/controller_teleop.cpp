...
public:
  ControllerTeleop()
  : Node("motor_controller"), manual_enabled_(true), robot_disabled_(false), left_wheel_motor_(
      "can0", 1), right_wheel_motor_("can0", 2), lift_actuator_motor_("can0", 3)
  {
...
private:
  ...
  {
    if (steam_mode_) {
      RCLCPP_INFO(get_logger(), "STEAMDECK MODE");
    } else if (xbox_mode_) {
      ...
      RCLCPP_ERROR(
        get_logger(),
        "\033[1;31mNO CONTROLLER SELECTED, CHECK LAUNCH PARAMETERS\033[0m");
    }
  }
  ...
  void joy_callback(const sensor_msgs::msg::Joy::SharedPtr joy_msg)
  {
    ...
    share_button_ = get_button(joy_msg, {9, 8, 9, 2});
    menu_button_ = get_button(joy_msg, {10, 9, 10, 13});
    home_button_ = get_button(joy_msg, {11, 10, 8, 11});
    a_button_ = get_button(joy_msg, {1, 0, 0, 3});
    b_button_ = get_button(joy_msg, {0, 2, 1, 4});
    x_button_ = get_button(joy_msg, {2, 3, 2, 5});
    y_button_ = get_button(joy_msg, {3, 2, 3, 6});
    left_bumper_ = get_button(joy_msg, {4, 4, 4, 20});
    right_bumper_ = get_button(joy_msg, {5, 5, 5, 21});
  ...
  {
    size_t index = steam_mode_ ? 3 : xbox_mode_ ? 2 : ps4_mode_ ? 1 : 0;
  ...
  bool steam_mode_, xbox_mode_, ps4_mode_, switch_mode_, outdoor_mode_;
  double speed_multiplier_, left_speed_, right_speed_, lift_actuator_speed_;
  bool home_button_, share_button_, menu_button_, a_button_, b_button_, x_button_, y_button_;
  double left_trigger_, right_trigger_, left_bumper_, right_bumper_, d_pad_vertical_,
    d_pad_horizontal_;
